...
const HomeFeature = ({ title, desc, image, alt, changeSequence }) => {
    ...
    return (
        <section className={`flex flex-col-reverse  rounded-md md:flex-row justify-center bg-white ${changeSequence && "md:flex-row-reverse"} `}>
            { changeSequence ? <Fade bottom>
                    <div className='space-y-3 max-w-lg  p-2 bg-white rounded-md shadow-sm sm:shadow-none flex flex-col justify-center '>
